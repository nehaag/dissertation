...
efficiently to justify their cost contribution. To cater the memory needs of
...
example is Intel/Micron's 3-D XPoint technology~\cite{xpoint}, which provides a slow
...
such as bandwidth or access cost per bit, which are critical in domains such as
GPGPU computing or data-centers. In this thesis we study memory characteristics
beyond memory access latency with the goal to simplify system design, easing
programmability, while being able to squeeze the best possible performance out
...
GPU can lead to a significant challenges because of design and verification
...
thus improving cross-vendor design cycle time.
...
Detection of such cold data must be done {\it application-transparently}, since
...
launch latency, resulting in exposure of data copy latency to GPU kernel run
time. Thus, systems with highest throughput requirements (i.e., majority of
GPGPU systems) still use programmer-optimized code to run on the GPUs.
...
Simple but effective page management algorithms have enormous durability, with no
...
\textbf{Cheaper Memory Technology in Cloud Deployments:} Slower but cheaper memory
technologies such as NVRAMs hold great promise to lower the memory costs for
cloud vendors. However, vendors have little control over the client
applications, and have to guarantee SLAs. Also, dependence on hardware features
only delays the adoption of such technologies due to the time required for
rolling out new features in CPUs. Thermostat is the first policy to our
knowledge that addresses these challenges by a software-only mechanism, thus
paving the way towards large-scale deployments of such technologies in
datacenters.
